...
- **EVs, IVs and nature** (`gamedata::training` for the game's rules, from `MonGainEVs`/`CalculateMonStats` in the decomp; `state::Training` per member; `sense::training` solves). The rules are:
  - EVs are 0–255 per stat and 510 in all, and every Pokémon is created with none. Each "X gained N EXP. Points!" page gives X the whole EV yield of the foe that fainted (the species named on the "Wild/Foe Y fainted!" page, else the opponent HUD's). This is not split like experience: an Exp. Share holder gets it too. A Macho Brace doubles it; Pokérus can't spread in FireRed.
  - Stats are computed only at a level-up (after that defeat's EVs are added), an evolution or a PC withdrawal. So each stat reading (summary Skills page, HUD max HP, and the level-up window's second page, `LevelUpStatsObserved`) is kept as a `StatSample` with the EV range of that computation.

  How the bot applies them:
  - A member obtained while watched starts exact at 0 EVs (`Training::obtained`).
  - A member whose history is unknown, a defeat whose foe wasn't read, or summary experience above the awards seen widens `unseen`. The solver then only uses the EVs as bounds.
  - `solve` intersects the IVs each sample allows (closed form per IV, O(1)) for each of the 25 natures. The nature read on the Info page's TRAINER MEMO ("LAX nature.") restricts them. The natures and IV ranges that remain become `IvsEstimated`.
  - If nothing fits, an award or a reading was wrong: the EVs' upper bounds are dropped and the estimate is marked `consistent: false`.
  - `agent::party::Member::stats` falls back to the estimate when the summary's reading is from an earlier level. The web UI shows EVs, IV ranges and natures per member.
