...
//! * [`EmulatorHid`] lets the WiFi controller firmware's device code
//!   (`pokebot_remote::Device`) press the emulator's buttons.
//!
//! The bot process then uses its production adapters (`capture-card`,
//! `pabotbase`, `esp32-wifi`) unchanged.
...
pub use wifi_device::EmulatorHid;
