...
#[test]
fn connection_loss_is_reported() {
    ...
    let server = std::thread::spawn(move || {
        ...
        writeln!(
            stream,
            r#"{{"type":"hello","name":"x","firmware":"0","protocol":2,"controller":"hori-pokken-wired"}}"#
        )
        .unwrap();
        // Then hang up.
    });
    let controller = Esp32WifiController::connect(Esp32WifiConfig::new(addr.to_string())).unwrap();
    server.join().unwrap();
